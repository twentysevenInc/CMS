...
<script type="text/javascript">
	function reloadDashboardPreview(){
		doAjax('components/lib/settings-dashboard.php', 'GET', '', function(data){
	  ...
	  $(function() {
	    $( "#settings-cms ul").sortable({
			update: function( event, ui ) {
				...
				doAjax('components/lib/updateDashboard.php', 'POST', 'data='+dash, function(data){
					reloadDashboardPreview();
				});
			}
		});
	...
	function deleteThisWidget(elem){
		elem = $(elem).parent();
		// console.log($(elem));
		doAjax('components/lib/deleteWidget.php', 'POST', 'data='+$(elem).attr('data')+'&pos='+$(elem).attr('pos'), function(d){
			$(elem).animate({
				'heigth':0,
				'opacity':0,
				'marginTop':0,
...
<div id="settings-cms">
	<h1>Dashboard</h1>
	<h4>Customize your own Dashboard.</h4>
	<a href="javascript:void(0);" onclick="javascript:navigateTo('components/lib/addWidget.php', undefined)" class="normal-button button" id="add-widget">Add Widget</a>
...
if(isset($_SESSION['id'])){
	...
	$query->prepare("
		SELECT widget.name, widget.id, plugin.name FROM userDashboard
		INNER JOIN widget ON (widget.id = userDashboard.widgetId)
