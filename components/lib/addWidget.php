...
<script type="text/javascript">
	function addWidgetToDashboard(elem){
		var el = $(elem).parent();
		doAjax('components/lib/insertWidget.php', 'POST', 'id='+el.attr('data'), function(data){
...
$db = new Database;
/*$result = $db->query('SELECT plugin.name FROM plugin
		INNER JOIN widget ON(plugin.id = widget.pluginid)
		INNER JOIN groupEnablesPlugin AS ge ON(plugin.id = ge.pluginid)
		WHERE ge.groupid = (SELECT groupId FROM user WHERE user.name = \''.$_SESSION['user'].'\')
		GROUP BY plugin.id ORDER BY plugin.name');*/
$result = $db->query('SELECT plugin.name FROM plugin
		INNER JOIN widget ON (plugin.id = widget.pluginid)
		INNER JOIN groupEnablesPlugin AS gep ON(gep.pluginId = plugin.id)
		INNER JOIN `group` AS gr ON (gep.groupId = gr.id)
		WHERE gr.id = (SELECT groupId FROM user WHERE user.name = \''.$_SESSION['user'].'\')
		GROUP BY plugin.id');
?>
...
while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
	...
	<div id="settings-widget-plugin">
		...
		<?php
			$res = $db->query('SELECT widget.name, widget.id FROM widget 
				INNER JOIN plugin ON widget.pluginid = plugin.id
