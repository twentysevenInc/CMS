...
<script type="text/javascript">
	...
	function ajaxUserDeleteRequest(){
		var ret = doAjax(
			'components/settings/manageusers/deleteUser.php', 
			'POST', 
...
<div class="settings-table-title">
	...
	<a href="javascript:void(0);" onclick="javascript:navigateTo('components/settings/manageusers/addUser.php', '#settings', undefined)" class="button normal-button">New User</a>
</div>
...
<?php
$db = new Database;
$result = $db->query("SELECT user.name AS name, user.avatar, g.name AS grname, user.email FROM user INNER JOIN `group` AS g ON (g.id = user.groupId) ORDER BY user.name");
...
<!-- <div class="manage-users-buttons">
	<a href="javascript:void(0);" onclick="javascript:navigateTo('components/settings/manageusers/addUser.php', '#settings', undefined)" class="button green-button">Add new User</a>
</div> -->
